...
func scanSwapRows(rows pgx.Rows) ([]domain.SwapEvent, error) {
	...
	for rows.Next() {
		var (
			e                                       domain.SwapEvent
			stationID                               *string
			retSoc, recSoc, temp, dod, dist, paidAmt *float64
			cycles                                  *int
		)
		if err := rows.Scan(&e.ID, &e.RiderID, &e.BatteryID, &stationID, &e.SwappedAt, &retSoc, &recSoc, &temp, &cycles, &dod, &dist, &paidAmt); err != nil {
			return nil, fmt.Errorf("queries: scan swap event: %w", err)
		}
		e.StationID = stationID
		e.ReturnedStateOfCharge = retSoc
		e.ReceivedStateOfCharge = recSoc
		e.ReturnedTemperatureC = temp
		e.ReturnedCycleCount = cycles
		e.ReturnedDepthOfDischarge = dod
		e.DistanceKmSinceLastSwap = dist
		e.PaidAmountKes = paidAmt
		out = append(out, e)
	}
...
func (s *Store) SwapEventsByRider(ctx context.Context, riderID string) ([]domain.SwapEvent, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT id, rider_id::text, battery_id::text, station_id, swapped_at, returned_state_of_charge, received_state_of_charge, returned_temperature_c, returned_cycle_count, returned_depth_of_discharge, distance_km_since_last_swap, paid_amount_kes
		FROM swap_events
		WHERE rider_id = $1
...
func (s *Store) SwapEventsByPartner(ctx context.Context, partnerID string) ([]domain.SwapEvent, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT se.id, se.rider_id::text, se.battery_id::text, se.station_id, se.swapped_at, se.returned_state_of_charge, se.received_state_of_charge, se.returned_temperature_c, se.returned_cycle_count, se.returned_depth_of_discharge, se.distance_km_since_last_swap, se.paid_amount_kes
		FROM swap_events se
